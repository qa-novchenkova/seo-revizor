 ...
 *
 * Последний аргумент — описание для людей. Оно намеренно отделено от meta:
 * meta.description читает модель, и от его точности зависит, правильно ли она
 * выберет инструмент. Переписывать его под читателя сайта нельзя.
 */
function tool(name, meta, run, human) {
  TOOLS.push({ name, title: meta.title, description: meta.description, ...human })
...
tool(
  ...
  {
    what:
      'Мгновенно тестирует конкретный URL: определяет код ответа, скорость загрузки, ключевые ' +
      'заголовки и анализирует полную цепочку редиректов.',
    why:
      'Помогает быстро узнать статус адреса, найти скрытые циклические перенаправления и оценить ' +
...
tool(
  ...
  {
    what:
      'Сканирует содержимое страницы: разбирает title, description, иерархию заголовков H1–H6, ' +
      'теги canonical, инструкции robots, микроразметку Open Graph и Schema.org, атрибуты alt у ' +
      'картинок.',
    why:
      'Проводит комплексный аудит конкретной страницы для улучшения её видимости и сниппета в ' +
      'поисковой выдаче.',
  }
)
...
tool(
  ...
  {
    what:
      'Скачивает и детально разбирает конфигурацию robots.txt: группирует правила, проверяет ' +
      'директивы Sitemap и контролирует, чтобы сайт не был случайно заблокирован для индексации.',
    why:
      'Базовый шаг любого SEO-аудита, который исключает критические ошибки доступа и помогает ' +
      'найти карту сайта.',
  }
)
...
tool(
  ...
  {
    what:
      'Проверяет sitemap.xml и вложенные индексы. Ищет дубли, чужие домены, мусорные параметры и ' +
      'формирует умную выборку — по одной странице каждого типа: главная, каталог, карточка, ' +
      'статья.',
    why:
      'Избавляет от рутины: вместо проверки сотен одинаковых страниц модель сразу тестирует ' +
      'представительные шаблоны сайта.',
  }
)
...
tool(
  ...
  {
    what:
      'Анализирует все ссылки на странице: находит битые, скрытые редиректы, ссылки без анкоров ' +
      'или с неинформативным текстом вроде «подробнее», а также атрибут nofollow. Извлекает ' +
      'реальную структуру разметки.',
    why:
      'Проверяет связность сайта и корректность передачи веса. Помогает понять, видят ли ' +
      'поисковые роботы ссылки на каталог или они скрыты ленивой загрузкой и скриптами.',
  }
)
...
tool(
  ...
  {
    what:
      'Проверяет склейку домена с www и без него, по HTTP и HTTPS, отсекает технические дубли ' +
      'главной страницы вроде /index.php, разбирает слэш на конце адреса и тестирует корректность ' +
      'ответа 404.',
    why:
      'Защищает сайт от расщепления веса между копиями. Если поисковая система видит несколько ' +
      'версий ресурса, остальные правки теряют смысл, поэтому шаг выполняется одним из первых.',
  }
)
...
tool(
  ...
  {
    what:
      'Сканирует SSL-сертификат, защитные заголовки, атрибуты cookie и смешанное содержимое. Ищет ' +
      'критические уязвимости: открытые папки, утечки логов ошибок и доступ к служебным файлам ' +
      'вроде .git или .env.',
    why:
      'Предотвращает взлом сайта и понижение в выдаче. Защита данных — важный фактор доверия, ' +
      'который классические SEO-аудиты часто упускают.',
  }
)
...
tool(
  ...
  {
    what:
      'Находит коды Яндекс.Метрики, Google Analytics и Tag Manager в разметке страниц. ' +
      'Контролирует их дублирование, сквозное присутствие на всём сайте и отсутствие синхронной ' +
      'загрузки, тормозящей сайт.',
    why:
      'Гарантирует точность собираемых данных. Без корректно настроенной аналитики невозможно ' +
      'оценить эффективность продвижения и понять, какие решения приносят результат.',
  }
)
...
tool(
  ...
  {
    what:
      'Проводит сравнение между страницами: ищет одинаковые метатеги, текстовые повторы, пустые ' +
      'или слишком короткие страницы, остатки шаблонов и расхождения в сквозных контактах, ' +
      'например разные телефоны.',
    why:
      'Помогает избавиться от мусорных страниц и дублей, которые размывают релевантность сайта и ' +
      'мешают продвижению целевых разделов.',
  }
)
...
tool(
  ...
  {
    what:
      'Обращается к Google PageSpeed Insights и оценивает ключевые метрики загрузки: LCP, CLS, ' +
      'INP и время отклика сервера. Определяет причины торможения — от блокирующих скриптов до ' +
      'неоптимизированной графики и отсутствия кеширования.',
    why:
      'Помогает поднять сайт выше в мобильной выдаче, снизить процент отказов и получить готовые ' +
      'технические рекомендации по ускорению страниц.',
  }
)
...
tool(
  ...
  {
    what:
      'Выгружает полную базу правил Ревизора: идентификаторы, критичность ошибок, формулировки, а ' +
      'также развёрнутые пояснения в формате «чем вредит» и «как исправить».',
    why:
      'Служит справочником и для агента, и для читателя отчёта: позволяет расшифровать любую ' +
      'найденную проблему и получить пошаговый порядок её устранения.',
  }
)
