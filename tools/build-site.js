...
function about() {
  const cards = [
    ['code', 'MCP-сервер', 'Одиннадцать инструментов, объявленных по протоколу Model Context Protocol. Каждый проверяет свой участок и возвращает данные: коды ответа, заголовки, ссылки, показатели скорости.'],
  ...
  <p class="note rise">${icon('eye', 'note__ico')}<span><b>Запускайте проверку только на своих
  сайтах или там, где есть разрешение владельца.</b> Обход создаёт нагрузку, а проверка
  безопасности запрашивает служебные файлы вроде <code>.git/config</code> — на чужом сайте
  это выглядит как поиск уязвимостей и в ряде стран приравнивается к сканированию.</span></p>
  </div></section>`
}
...
function chatMock() {
  ...
  const rows = steps
    .map(([name, time], i) => {
      const y = 78 + i * 22
      const wait = (0.2 + i * 0.15).toFixed(2)
  ...
  const files = [
    ...
    .map(([name, size, hint], i) => {
      const y = 392 + i * 50
      const wait = (2.4 + i * 0.22).toFixed(2)
  ...
  <svg viewBox="0 0 400 546" fill="none">
    ...
    <g class="chat__row" style="--at:1.85s">
      <text class="chat__done" x="26" y="318">готово за 1 мин 5 с</text>
    </g>
    <g class="chat__row" style="--at:2.1s">
...
function botSection() {
  ...
  const modes = [
    [
      'bot',
      'С моделью',
      'Порядок и состав проверок определяются динамически на основе контекста. LLM-агент анализирует промежуточные результаты и адаптирует граф шагов на лету. Система самостоятельно формирует репрезентативную выборку целевых страниц и генерирует кастомизированные выводы на естественном языке, предоставляя развёрнутый аудит вместо статических кодов ошибок.',
      ['API-ключ доступа к модели', 'от 12 ₽ за проверку', '1–1,5 минуты'],
    ],
    [
      'code',
      'Без модели',
      'Задействуются те же 11 инструментов и 113 правил, но в строго фиксированном порядке. Алгоритм работает без элементов генеративности: данные собираются напрямую из ответов утилит, а итоговое заключение компилируется по жёстко заданному текстовому шаблону.',
      ['аренда сервера', 'стоимость выбранного сервера', '≈ 30 секунд'],
    ],
  ]
    .map(
  ...
  const prices = [
    ['Haiku 4.5', '135 / 1080 ₽ за млн', '11,83 ₽', 'дешёвая, но обходит список не целиком'],
    ['Sonnet 5', '405 / 2025 ₽ за млн', '12,56 ₽', 'вызывает инструменты пачками, поэтому токенов тратит втрое меньше'],
    ['Opus', '675 / 3375 ₽ за млн', '≈ 90 ₽', 'для обхода чек-листа избыточна, цифра оценочная'],
  ]
    .map(
  ...
  <div class="botgrid">
    ...
    <div class="botside">
      <h3 class="sub sub--tight rise">Меню бота</h3>
      <div class="keys">${keys}</div>

      <h3 class="sub rise">Команды</h3>
      <p class="rise">Быстрые проверки отвечают за секунды и к модели не обращаются, поэтому
      бесплатны. У них свой счётчик: перепроверять одно место после правки можно сколько угодно,
      запас полных аудитов на день от этого не тратится.</p>
      <ul class="cmds rise">${commands}</ul>
    </div>
  </div>

  <h3 class="sub rise">Режимы оркестрации проверок</h3>
  <p class="intro intro--wide rise">Система поддерживает два режима выполнения проверок, переключение
  между которыми зависит от наличия API-ключа языковой модели (LLM) в конфигурационных настройках:
  детерминированный — фиксированный последовательный пайплайн в предустановленном статическом
  порядке; агентный — динамическая оркестрация, где LLM-агент сам определяет последовательность
  и приоритет запускаемых проверок.</p>

  <div class="modes">${modes}</div>

  <p class="note rise">${icon('eye', 'note__ico')}<span>В обоих режимах вызываются идентичные базовые
  методы валидации. Различие заключается исключительно в механизме управления очередью выполнения:
  статический препроцессинг против динамического планирования агентной моделью.</span></p>

  <h3 class="sub rise">Какие страницы попадают в аудит</h3>
  <p class="intro intro--wide rise">Проверяется не весь сайт, а выборка: главная и по одному образцу
  каждого типа страницы — раздел, карточка, статья, служебная. Типы различаются по форме адреса:
  <code>/catalog</code> и <code>/catalog/shoes</code> — разные типы, а <code>/catalog/shoes</code>
  и <code>/catalog/bags</code> — один и тот же. Страницы одного типа собраны по общему шаблону,
  поэтому ошибка разметки повторяется на всех сразу и одного образца хватает, чтобы её увидеть.
  В детерминированном режиме выборка фиксирована — пять страниц; в агентном её состав определяет
  модель.</p>

  <h3 class="sub rise">Сколько стоит прогон</h3>
  <p class="intro intro--wide rise">Расчёт стоимости за одну проверку (тестирование на идентичной
  целевой базе) коррелирует не со стоимостью токенов конкретной модели, а с суммарным количеством
  итераций в диалоге. Поскольку на каждом новом шаге контекст переписки передаётся повторно, модели
  с поддержкой параллельного вызова инструментов генерируют меньше запросов и обходятся экономически
  выгоднее, чем модели с последовательным пошаговым вызовом.</p>
  ...
  <p class="intro intro--wide rise">Для обеспечения непрерывной доступности бот разворачивается
  на облачном сервере. Подойдёт минимальный тарифный план с 1 ГБ оперативной памяти.</p>

  <ol class="steps rise">
    <li><b>Регистрация бота.</b> Создать бота и получить API-токен в Telegram через
    <code>@BotFather</code>, команда <code>/newbot</code>.</li>
    <li><b>Подготовка сервера.</b> Установить Node.js версии 22 или новее и браузер Chrome —
    он нужен для генерации PDF.</li>
    <li><b>Настройка файла подкачки.</b> Обязательный шаг: на тарифах с 1 ГБ памяти сборка
    PDF-отчёта без подкачки гарантированно упирается в нехватку памяти.</li>
    <li><b>Конфигурация окружения.</b> Склонировать репозиторий с GitHub, создать в корневой
    директории файл <code>.env</code> и вписать туда токен бота.</li>
    <li><b>Автозапуск процесса.</b> Настроить системную службу, чтобы бот поднимался после
    перезагрузки сервера или сбоя приложения.</li>
  </ol>
...
function startSection() {
  ...
  <p class="rise">Перед стартом убедитесь, что установлен Node.js 22 или новее. Для сборки PDF
  дополнительно нужен браузер — Chrome, Chromium или Edge; на сервере без графической оболочки
  его ставят отдельно. Без браузера ничего не ломается: отчёт приходит в Markdown и HTML,
  а PDF просто не собирается. Откройте терминал и выполните базовые команды установки:</p>
...
npm install</code></pre>
  ...
  <h3 class="sub rise">${icon('bot', 'h2__ico')}Какая модель подойдёт</h3>
  <p class="rise">Агент целиком построен на вызове инструментов, поэтому модель обязана его
  поддерживать: без этого он не работает вовсе, а не работает хуже. По умолчанию стоит
  <b>Claude Sonnet 5</b> — он вызывает инструменты пачками и проходит чек-лист целиком.</p>

  <p class="rise">Дешёвая модель цикл не ломает, но ведёт себя иначе: вызывает инструменты
  по одному и берёт нижнюю границу выборки страниц. Кругов диалога выходит больше, на каждом
  круге переписка отправляется заново — и токенов уходит втрое больше при той же итоговой цене.
  Замеры на одном сайте: Sonnet 5 — 19 672 входящих токена и пять страниц из пяти;
  Haiku 4.5 — 70 873 токена и две страницы из пяти.</p>
...
.hero h1 {
  ...
  /* Отрицательная разрядка сдвигает буквы влево, и последняя выходит за
     границу блока, по которой обрезается градиент. Минус три сотых —
     предел, при котором «р» ещё цела. */
...
/* Первый подзаголовок в колонке рядом с картинкой: сверху отступ не нужен,
   иначе колонка начинается заметно ниже соседней. */
